...
double GetBestRelativeDA(const std::vector<std::pair<std::string, tf::Pose>>& gt_artifacts,
                         const std::vector<std::pair<std::string, tf::Stamped<tf::Point>>> &report,
                         std::vector<int>* best_da,
                         Eigen::MatrixXf* best_transform) {
  std::vector<std::pair<std::string, tf::Point>> gt_artifact_points;
  for (size_t i = 0; i < gt_artifacts.size(); i++) {
    gt_artifact_points.push_back(std::make_pair(gt_artifacts[i].first, gt_artifacts[i].second.getOrigin()));
  }
  return GetBestRelativeDA(gt_artifact_points,
                           report, best_da, best_transform);
}
